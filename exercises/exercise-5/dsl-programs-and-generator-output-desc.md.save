...
This program runs on two Raspberry Pis connected over WiFi via MQTT. Pi5 reads sensors and publishes events. Pi4 subscribes, evaluates 
...
**`coordinator.ex`** — A stateful GenServer acting as a rule engine. It maintains an in-memory map of the last known state of every sensor topic (e.g. `%{temperature: :low, motion: :still}`). On every update it evaluates all DSL rul
z
